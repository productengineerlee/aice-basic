...
import { AlertTriangle, Bookmark, ChevronLeft, ChevronRight, Clock3, Download, FileQuestion, Menu, Save, Send, X } from "lucide-react";
...
export function ExamRunner({ exam }: { exam: PublicExam }) {
  ...
  const sectionTitleByCode = useMemo(() => new Map(exam.sections.map((section) => [section.code, section.title])), [exam.sections]);
...
const finish = useCallback(async () => {
  ...
  return <main className="runner"><header className="runner-header"><div className="runner-brand"><Link href="/" aria-label="랜딩 페이지"><Image src="/logo-mark.png" alt="AICE LAB" width={28} height={28} className="brand-mark"/><b>AICE LAB</b></Link><span>{exam.title}</span></div><div className="runner-header-actions"><Link className="sample-link" href="/exams"><FileQuestion/><span>샘플문제</span></Link><div className={`timer ${remaining < 300 ? "danger" : ""}`}><Clock3/><span>남은 시간</span><b>{formatTime(remaining)}</b></div><button className="nav-toggle" onClick={() => setNavOpen(true)} aria-label="문항 목록"><Menu/></button></div></header><div className="runner-progress"><i style={{ width: `${progress}%` }}></i></div><div className="runner-layout"><aside className={navOpen ? "open" : ""}><div className="aside-head"><div><span>답안 현황</span><b>{answered} / {exam.questionCount}</b></div><button onClick={() => setNavOpen(false)}><X/></button></div><div className="number-grid">{exam.questions.map((item, index) => <button key={item.number} onClick={() => move(index)} className={`${index === current ? "current" : ""} ${answers[String(item.number)] ? "answered" : ""} ${flagged.includes(item.number) ? "flagged" : ""}`}><span>{item.number}</span>{flagged.includes(item.number) && <Bookmark/>}</button>)}</div><div className="legend"><span><i className="done"></i>답변 완료</span><span><i className="flag"></i>검토 표시</span></div><div className="autosave"><Save/><div><b>자동 저장</b><span>{saveCopy}</span></div></div>{exam.dataset && <a className="runner-download" href={`/api/exams/${exam.slug}/dataset`}><Download/>실습 데이터 다시 받기</a>}</aside><section className="question-area"><div className="question-top"><div><span className="section-label">{sectionTitleByCode.get(question.section) ?? question.section}</span><span className="score-label">{question.score}점</span></div><button className={flagged.includes(question.number) ? "active" : ""} onClick={toggleFlag}><Bookmark/>검토 표시</button></div><div className="question-number">문제 {question.number}<small> / {exam.questionCount}</small></div><p className="question-prompt">{formatPrompt(question.prompt)}</p>{question.imageUrl && <img className="question-image" src={question.imageUrl} alt={`문제 ${question.number} 참고 이미지`}/>}<div className="answer-area">{question.choices.length > 0 ? question.choices.map((choice) => <label className={`choice ${answers[String(question.number)] === choice.id ? "selected" : ""}`} key={choice.id}><input type="radio" name={`q-${question.number}`} value={choice.id} checked={answers[String(question.number)] === choice.id} disabled={remaining === 0 || submitting} onChange={() => setAnswer(choice.id)}/><span className="choice-label">{choice.label}</span><span>{choice.content}</span></label>) : <label className="text-answer"><span>답안 입력</span><input inputMode={question.type === "text" ? "text" : "decimal"} value={answers[String(question.number)] ?? ""} disabled={remaining === 0 || submitting} onChange={(event) => setAnswer(event.target.value)} placeholder={question.type === "unit_value" ? "예: 12.9k" : "정답을 입력하세요"}/><small>문제에서 안내한 반올림 및 단위 형식을 지켜 입력하세요.</small></label>}</div><div className="question-actions"><button disabled={current === 0} onClick={() => move(current - 1)}><ChevronLeft/>이전 문제</button>{current < exam.questions.length - 1 ? <button className="next" onClick={() => move(current + 1)}>다음 문제<ChevronRight/></button> : <button className="submit" onClick={() => setShowSubmit(true)}>답안 제출<Send/></button>}</div></section></div>{showSubmit && <div className="modal-backdrop"><div className="submit-modal"><AlertTriangle/><h2>{remaining === 0 ? "시험 시간이 종료되었습니다" : "답안을 제출할까요?"}</h2><p>총 {exam.questionCount}문항 중 <b>{answered}문항</b>에 답했습니다. 미응답 {exam.questionCount - answered}문항이 있습니다.</p>{submitError && <p className="submit-error">{submitError}</p>}<div><button onClick={() => setShowSubmit(false)} disabled={remaining === 0 || submitting}>계속 풀기</button><button className="confirm" onClick={() => void finish()} disabled={submitting}>{submitting ? "채점·저장 중..." : submitError ? "다시 제출" : "최종 제출"}</button></div></div></div>}</main>;
}