  ...
  return <main className="result-page">
    ...
    <section className="result-container">
      <div className="result-grid">
        <article className="result-card"><h2>영역별 점수</h2><div className="section-bars">{result.sections.map(section => <div key={section.code}><b>{section.earnedScore}<small>/{section.maxScore}</small></b><div className="bar-track"><i style={{ height: `${section.percentage}%` }} /></div><span>{section.title}</span></div>)}</div></article>
      ...
      <div className="review-heading"><div><h2>문항별 결과와 해설</h2></div><Link href={`/exams/${slug}/take`}><RotateCcw />다시 풀기</Link></div>
