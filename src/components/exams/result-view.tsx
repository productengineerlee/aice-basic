...
export function ResultView({ slug }: { slug: string }) {
  ...
  if (!result) return <main className="result-empty"><BarChart3 /><h1>{loading ? "Supabase에서 결과를 불러오는 중" : loadError ? "결과를 불러오지 못했습니다" : "채점 결과가 없습니다"}</h1><p>{loading ? "잠시만 기다려 주세요." : loadError ? "네트워크 상태를 확인하고 새로고침해 주세요." : "모의고사를 제출하면 결과가 이곳에 표시됩니다."}</p><Link href={`/exams/${slug}`}>시험 안내로 돌아가기</Link></main>;
