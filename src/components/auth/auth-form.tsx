...
export function AuthForm({ mode, action }: Props) {
  ...
  return (
    <div className="auth-card">
      <div className="auth-heading"><h1>{copy[mode].title}</h1><p>{copy[mode].sub}</p></div>
      <form action={formAction} className="auth-form">
        {emailNeeded && <label>이메일<input name="email" type="email" autoComplete="email" placeholder="name@example.com" required /></label>}
        {mode === "signup" && <label>생년월일<input name="birthDate" type="date" required /></label>}
        {passwordNeeded && <label>{mode === "update" ? "새 비밀번호" : "비밀번호"}<input name="password" type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} placeholder="8자 이상 입력" minLength={8} required /></label>}
        {(mode === "signup" || mode === "update") && <label>비밀번호 확인<input name="confirmPassword" type="password" autoComplete="new-password" placeholder="비밀번호 다시 입력" minLength={8} required /></label>}
        {mode === "signup" && <div className="agreements"><label><input name="terms" type="checkbox" required/><span><b>[필수]</b> 이용약관에 동의합니다.</span></label><label><input name="privacy" type="checkbox" required/><span><b>[필수]</b> 개인정보 수집·이용에 동의합니다.</span></label></div>}
        ...
        <button className="auth-submit" disabled={pending}>{pending ? "처리 중..." : copy[mode].button}</button>
      </form>
