...
async function main() {
  ...
  const slug = "korean-history-72-simhwa";
  const title = "제72회 한국사능력검정시험 심화 기출문제";
  ...
  const questions = JSON.parse(fs.readFileSync(path.join(root, "data", "korean-history", "72-simhwa.json"), "utf8"));
  ...
  const { data: exam, error: examErr } = await admin
    ...
    .insert({
      slug,
      title,
      description: "제72회 한국사능력검정시험 심화 등급 기출문제(50문항, 100점)입니다.",
      kind: "quiz",
      status: "published",
      duration_minutes: 80,
      passing_score: 60,
  ...
  console.log(`${slug}: seeded 50 questions (exam id ${exam.id}). Run update_korean_history_72_images.mjs separately to attach question images.`);
}
