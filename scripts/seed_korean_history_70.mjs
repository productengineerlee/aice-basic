...
async function main() {
  ...
  const slug = "korean-history-70-simhwa";
  const title = "제70회 한국사능력검정시험 심화 기출문제";
  ...
  const questions = JSON.parse(fs.readFileSync(path.join(root, "data", "korean-history", "70-simhwa.json"), "utf8"));
  ...
  const { data: exam, error: examErr } = await admin
    ...
    .insert({
      slug,
      title,
      description: "제70회 한국사능력검정시험 심화 등급 기출문제(50문항, 100점)입니다.",
      kind: "quiz",
      status: "published",
      duration_minutes: 80,
      passing_score: 60,
      ...
      // 70회는 71회보다 먼저 치러진 회차이므로, 71회(2026-08-26) 이전 시각으로
      // published_at을 설정해 "회차 내림차순(최신 우선)" 목록에서 70회가 71회 아래에 오도록 한다.
  ...
  console.log(`${slug}: seeded 50 questions (exam id ${exam.id}). Run update_korean_history_70_images.mjs separately to attach question images.`);
}
