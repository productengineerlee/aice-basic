...
async function makeSession() {
  ...
  const email = `codex-gongin2cha-${stamp}@example.invalid`;
...
async function run(label, buildAnswers) {
  ...
  try {
    ...
    for (const section of submitted.sections) console.log(`  section ${section.code}: earned=${section.earnedScore} min=${section.minScore} correct=${section.correctCount}/${section.questionCount}`);
...
await run("ALL CORRECT (expect passed=true, 300/300)", (questions, keyByQuestion, choiceById) => {
  const answers = {};
...
await run("ONE SECTION ALL WRONG (expect passed=false despite total >= passingScore)", (questions, keyByQuestion, choiceById) => {
  const answers = {};
