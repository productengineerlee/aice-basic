...
async function main() {
  ...
  const title = "제76회 한국사능력검정시험 심화 기출문제";
  ...
  const questions = JSON.parse(fs.readFileSync(path.join(root, "data", "korean-history", "76-simhwa.json"), "utf8"));
  ...
  const { data: exam, error: examErr } = await admin
    ...
    .insert({
      slug,
      title,
      description: "제76회 한국사능력검정시험 심화 등급 기출문제(50문항, 100점)입니다.",
      kind: "quiz",
      status: "published",
      duration_minutes: 80,
      passing_score: 60,
